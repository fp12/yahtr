...
class UnitActionButton(Button):
    def __init__(self, index, action_type, rk_skill=None, **kwargs):
        self.index = index
...
class ActionsBar(BoxLayout):
    ...
    def create_action_widget(self, index, action_type, text, rk_skill=None):
        new_widget = UnitActionButton(index=index, action_type=action_type, text=text, rk_skill=rk_skill)
        ...
        for a in action_node:
            if a.data in [ActionType.Weapon, ActionType.Skill]:
                for rk_skill in unit.get_skills(a.data):
                    self.create_action_widget(index=index, action_type=a.data, text=rk_skill.skill.name, rk_skill=rk_skill)
                    index += 1
